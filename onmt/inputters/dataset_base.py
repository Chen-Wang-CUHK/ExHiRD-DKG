...
class DatasetBase(Dataset):
    ...
    def __init__(self, fields, src_examples_iter, title_examples_iter, tgt_examples_iter,
                 filter_pred=None):

        dynamic_dict = 'src_map' in fields and 'alignment' in fields

        if tgt_examples_iter is not None:
            if title_examples_iter is not None:
                examples_iter = (self._join_dicts(src, title, tgt) for src, title, tgt in
                                 zip(src_examples_iter, title_examples_iter, tgt_examples_iter))
            else:
                examples_iter = (self._join_dicts(src, tgt) for src, tgt in
                                 zip(src_examples_iter, tgt_examples_iter))
        else:
            if title_examples_iter is not None:
                examples_iter = (self._join_dicts(src, title) for src, title in
                                 zip(src_examples_iter, title_examples_iter))
            else:
        ...
        for ex_dict in examples_iter:
            ...
            # add by wchen for src_sent_position field
            if "src_sent_position" in fields:
                ex_dict["src_sent_position"] = ex_dict["src"]
            # add by wchen for src_word_sent_ids field
            if "src_word_sent_ids" in fields:
                ex_dict["src_word_sent_ids"] = ex_dict["src"]
    ...
    def _dynamic_dict(self, example, src_field, tgt_field):
        ...
        # changed by wchen, for hr_src and hr_tgt
        if type(src_field) is not NestedField:
            ...
            # for hr_src
            src_sents = src_field.tokenize(example["src"])
            src = [src_field.nesting_field.tokenize(sent) for sent in src_sents]
            src_vocab = Vocab(Counter(list(itertools.chain.from_iterable(src))), specials=[unk, pad])
            # Map source tokens to indices in the dynamic dict.
            src_map = [torch.LongTensor([src_vocab.stoi[w] for w in sent]) for sent in src]

        example["src_map"] = src_map

        if "tgt" in example:
            if type(tgt_field) is not NestedField:
                tgt = tgt_field.tokenize(example["tgt"])
                mask = torch.LongTensor(
                    [0] + [src_vocab.stoi[w] for w in tgt] + [0])
            else:
                # for hr_tgt
                tgt_sents = tgt_field.tokenize(example["tgt"])
                tgt = [tgt_field.nesting_field.tokenize(sent) for sent in tgt_sents]
                # sent: ["<p/a_start>", "key", "phrase", ";", "<blank>"]
                # [0] here is for the "<s>" token
                mask = [torch.LongTensor([0] + [src_vocab.stoi[w] for w in sent[:-2]] + [0, 0]) for sent in tgt]
                # for ["<s>", "</s>"]
                mask.append(torch.LongTensor([0, 0]))
